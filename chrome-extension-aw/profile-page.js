            ...
            <div class='${divClass}'>
                <img class='ku_gallerywrapper_img' style='max-width:${maxWidth}px;' src='${src}' onclick="window.open('${src}')" data-file-name="${fileName}"/>
                    ...
                    <span class="emoji">${emoji}</span>
                        <button onclick="window.open('https://yandex.com/images/search?rpt=imageview&url=${encodeURIComponent(src)}');window.open('https://www.google.com/searchbyimage?&image_url=${encodeURIComponent(src)}');window.open('https://www.bing.com/visualsearch/Microsoft/SimilarImages?&imgurl=${encodeURIComponent(src)}')">Image Search</button>
                    </div>                    
            </div>
        </div>`;
    }
